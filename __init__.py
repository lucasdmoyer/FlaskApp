...
engine = create_engine('postgresql://catalog:password@localhost/item')
...
# Assumes that there are no items of the same name in different categories
...
def showLogin():
    ...
    # attackers would have to guess this code in order to make a request on
    # the user's behalf
    # later we check if the user and the login session still have the same
    # state value when a user tries to authenticate
    ...
    # loads to login button, but still need to let the server know that the
...
def showItems(category_name):
    ...
    numberOfItems = \
        session.query(Item).filter_by(category=category).count()
...
@app.route(
    '/catalog/<string:item_name>/delete', methods=['GET', 'POST'])
